                                        ...
                                        <v-card-text class="mt-2">
                                            <v-row dense>
                                                <v-col cols="12" sm="12" md="4" lg="4">
                                                    <v-select v-model="item.orcamentoId" label="Orçamentos"
                                                        :rules="fieldRules" item-value="id" item-text="descricao"
                                                        :items="orcamentos" outlined dense />
                                                </v-col>
                                                <v-col cols="12" sm="12" md="4" lg="4">
                                                ...
                                                <v-col cols="12" sm="12" md="4" lg="4">
                                                    <v-text-field v-model="item.dataInicio" label="Data de Inicio"
                                                        :rules="fieldRules" :counter="60" dense outlined />
                                                </v-col>
                                                <v-col cols="12" sm="12" md="4" lg="4">
                                                    <v-text-field v-model="item.dataFinalizacao" label="Data de Finalização"
                                                        :rules="fieldRules" dense outlined />
                                                </v-col>
                                                <v-col cols="12" sm="12" md="4" lg="4">
                                                    <v-text-field v-model="item.valor" label="Valor do Serviço"
                                                        :rules="fieldRules" dense outlined />
                                                </v-col>
                                            </v-row>
                                        </v-card-text>
...
import { Orcamento, Servico, Status } from '@/core/models/geral';
import { OrcamentoService, ServicoService, StatusService } from '@/core/services/geral';
import { AlertaSimplesErro, AlertaSimplesResponse } from '@/components/shared/AlertService';

@Component
export default class CadastroServico extends PageBase {
    ...
    isOrcamentoLoading: boolean = false;
    onSearchOrcamento: any = null;
    ...
    @Watch('onSearchOrcamento')
    SearchOrcamento(val: string) {
        if (this.item.orcamentoId) return;
        if (this.isOrcamentoLoading) return;
        if (!val) return;
        this.isOrcamentoLoading = true
        this.orcamentoService.AutoComplete(val)
            .then(
                res => {
    ...
    mounted() {
        ...
        this.orcamentoService.ListarTudo().then(
            res => {
