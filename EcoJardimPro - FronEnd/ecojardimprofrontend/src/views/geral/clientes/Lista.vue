<template>
  <master-page titulo="Clientes" icone="mdi-account-group-outline">
...
import { ClienteService, UsuarioService } from '../../../core/services/geral'
...
@Component
export default class ListaClientes extends PageBase {
  ...
  Atualizar(isFiltro: boolean = false) {
    ...
    this.service.Listar(page, itemsPerPage, sortBy, sortDesc, this.search, this.header,undefined, 'Projetos').then(
      res => {
  ...
  AbrirDialogCadastro(item?: Cliente) {
    if(item){
        this.service.ObterPorId(item.id).then(
            res => {
                this.item = new Cliente(res.data);
            },
            err => {
                AlertaSimplesErro("Aviso", err);
            }
